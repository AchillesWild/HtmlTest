        ...
        $.ajax({
            ...
            success: function(obj) {
                ...
                for (let i = 0; i < count; i++) {
                    num++;
                    let goodsTradeMonth = list[i];
                    var id = goodsTradeMonth['id'];
                    var month = goodsTradeMonth['month'];
                    var incomeAmountValue = goodsTradeMonth['incomeAmountValue'];
                    if (incomeAmountValue != 0) {
                        incomeAmountValue = "<a href='javascript:void(0)' title='点击查看详细' onclick='getClassified(\""+month+"\",1)'>"+incomeAmountValue+"</a>";
                    }
                    var payAmountValue = goodsTradeMonth['payAmountValue'];
                    if (payAmountValue != 0) {
                        payAmountValue = "<a href='javascript:void(0)' title='点击查看详细' onclick='getClassified(\""+month+"\",0)'>"+payAmountValue+"</a>";
                    }
                    var marginValue = goodsTradeMonth['marginValue'];
//                    var createTimeStr = goodsTradeMonth['createTimeStr'];
                    var updateTimeStr = goodsTradeMonth['updateTimeStr'];
                    var bgColor;
                    if (i % 2 == 0) {
                         bgColor = '#DCDCDC';
                    } else {
...
//                                   + "<td>"+createTimeStr+"</td>"
                                   + "<td>"+updateTimeStr+"</td>"
                                   + "<td><button onclick='toSum(\""+month+"\")' style='margin-right: 4px'>重新汇总</button>"
                                       + "<button onclick='exportMonthDetail(\""+month+"\")' style='margin-right: 4px'>导出明细Excel</button>"
                                   + "</td>"
                           + "</tr>";
    ...
    function toSumRecent6 () {
        $(this).attr('disabled',"disabled");
        $.ajax({
            url: tradeMonthGoodsHost + "/sum/recent/6",
            method: 'GET',
            success: function(obj) {
    ...
    function toSumRecent2 () {
     ...
     function getClassified(month,flowType) {
        openMask();
        $.ajax({
            url: tradeMonthGoodsHost + "/classified/list?month="+month+"&flowType="+flowType,
            method: 'GET',
            success: function(obj) {
                var flowTypeValue;
                if(flowType == 0){
    ...
    function exportExcel(){
        ...
        var name  = desc + '进货/销售(月度)';
        exportExcelFile(tradeMonthGoodsHost + '/download?'+ $("#search").serialize(), name + '.xlsx');
    }

    function exportMonthDetail(month){
        exportExcelFile(tradeMonthGoodsHost + "/download/detail?month=" + month, month + '进货/销售(明细).xlsx');
    }